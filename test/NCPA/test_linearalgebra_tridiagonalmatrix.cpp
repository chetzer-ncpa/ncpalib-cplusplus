...
#define _TEST_TITLE_    NCPALinearAlgebraLibraryTridiagonalMatrixTest

typedef double test_t;
typedef tridiagonal_matrix<test_t> mat_t;
...
class _TEST_TITLE_ : public ::testing::Test {
    protected:
        void SetUp() override {  // define stuff here
            ...
            more_rows.set_diagonal( -2.0 ).set_diagonal( 1.0, 1 ).set_diagonal(
                1.0, -1 );

            more_cols = mat_t( dim1, dim2 );
            more_cols.set_diagonal( -2.0 ).set_diagonal( 1.0, 1 ).set_diagonal(
                1.0, -1 );
            ...
            symmetric.set_diagonal( -2.0 ).set_diagonal( 1.0, 1 ).set_diagonal(
                1.0, -1 );
...
TEST_F( _TEST_TITLE_, CopyOfDenseMatrixIsTridiagonal ) {
    ...
    EXPECT_TRUE( bdmat.is_tridiagonal() );
...
TEST_F( _TEST_TITLE_, GetDiagonalReturnsExpectedValues ) {
    std::vector<test_t> diag0( { -2, -2, -2, -2, -2 } ),
        diag1( { 1.0, 1.0, 1.0, 1.0 } ),
        diag_1( { 1.0, 1.0, 1.0, 1.0 } );
...
TEST_F( _TEST_TITLE_, MatrixMatrixMultiplicationIsCorrect ) {
    ...
    lhs.zero()
        ...
        .set_diagonal( { 5, 4, 3, 2 }, -1 );
    mat_t rhs = lhs;
    rhs.set_diagonal( { 10, 11, 12, 13 }, -1 );
    // cout << "LHS: " << endl << lhs << "RHS: " << endl << rhs;
    band_diagonal_matrix<test_t> bdproduct( dim1, dim1 );
    bdproduct.set_diagonal( { 61, 111, 133, 157, 43 } )
        .set_diagonal( { 18, 35, 56, 81 }, 1 )
        .set_diagonal( { 42, 56, 72 }, 2 )
        .set_diagonal( { 25, 41, 57, 73 }, -1 )
        .set_diagonal( { 40, 33, 24 }, -2 );
    // cout << "Expect product = " << endl << product2;
    auto product = lhs.multiply( rhs );
    // cout << "Actual product = " << endl << *product;
    EXPECT_TRUE( bdproduct.equals( *product ) );
}
...
TEST_F( _TEST_TITLE_, SetDiagonalWorks ) {
    ...
    control.set( 0, 1, testval );
    control.set( 1, 2, testval );
    control.set( 2, 3, testval );
    control.set( 3, 4, testval );
    control.set( 1, 0, -testval );
    control.set( 2, 1, -testval );
    control.set( 3, 2, -testval );
    control.set( 4, 3, -testval );

    // first version
    test_t diag[ 5 ] = { testval, testval, testval, testval, testval };
    test_t ndiag[ 5 ] = { -testval, -testval, -testval, -testval, -testval };
    vector<test_t> vdiag( { testval, testval, testval, testval, testval } );
    vector<test_t> nvdiag( { -testval, -testval, -testval, -testval, -testval } );
    
    testmat.set_diagonal( 5, diag );
    testmat.set_diagonal( 4, diag, 1 );
    testmat.set_diagonal( 4, ndiag, -1 );
    ...
    testmat.set_diagonal( vdiag, 1 );
    // cout << "Upper second off-diagonal set" << endl;
    testmat.set_diagonal( nvdiag, -1 );
    ...
    testmat.set_diagonal( { testval, testval, testval, testval, testval }, 1 );
    testmat.set_diagonal( { -testval, -testval, -testval, -testval, -testval },
                          -1 );
    EXPECT_TRUE( testmat.equals( control ) );
    ...
    testmat.set_diagonal( testval, 1 );
    testmat.set_diagonal( -testval, -1 );
    EXPECT_TRUE( testmat.equals( control ) );
}
