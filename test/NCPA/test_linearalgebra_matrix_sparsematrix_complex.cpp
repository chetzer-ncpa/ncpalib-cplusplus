...
#define _TEST_TITLE_    NCPALinearAlgebraMatrixComplexSparseMatrixTest
...
class _TEST_TITLE_ : public ::testing::Test {
    protected:
        ...
        // declare stuff here
        details::sparse_matrix<test_t> square, dproduct;
...
TEST_F( _TEST_TITLE_, ResizeWorks ) {
    ...
    EXPECT_THROW( { wrapper1[2][2]; }, std::range_error );
}
...
TEST_F( _TEST_TITLE_, SetRowMethodsWork ) {
    ...
    for ( auto i = 0; i < dim1; i++ ) {
        _TEST_EQ_( wrapper2.get( 1, i ), wrapper1[0][i] );
    }
    wrapper2 = wrapper1;
...
TEST_F( _TEST_TITLE_, MultiplyWorks ) {
    ...
    // std::cout << "Resizing mat2 to [3,5]" << endl;
    mat2.resize( 3, 5 ).set( 1.0 );
    // std::cout << "Resizing mat3 to [5,3]" << endl;
    mat3.resize( 5, 3 ).set( 1.0 );
    // std::cout << "Resizing product to [5,5]" << endl;
    product.resize( 5, 5 ).set( 3.0 );
    // std::cout << "Multiplying" << endl;
    EXPECT_TRUE( mat3.multiply(mat2) == product );
    // std::cout << "Resizing mat2 to [3,5]" << endl;
    mat2.resize( 3, 5 ).set( 1.0 );
    // std::cout << "Resizing mat3 to [5,2]" << endl;
    mat3.resize( 5, 2 ).set( 1.0 );
    // std::cout << "Resizing product to [3,2]" << endl;
