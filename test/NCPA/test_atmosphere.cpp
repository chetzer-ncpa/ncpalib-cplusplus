...
TEST_F( _TEST_TITLE_, DependentValuesAreCorrect ) {
    EXPECT_TRUE( test_atmos.get_property_units( "T" )->equals( NCPA::units::KELVIN ) );
    EXPECT_TRUE( test_atmos.get_property_units( "U" )->equals( NCPA::units::METERS_PER_SECOND ) );
    EXPECT_TRUE( test_atmos.get_property_units( "V" )->equals( NCPA::units::METERS_PER_SECOND ) );
...
TEST_F( _TEST_TITLE_, UnitsCanBeConverted ) {
    ...
    EXPECT_TRUE( test_atmos.get_property_units( "U" )->equals( NCPA::units::KILOMETERS_PER_SECOND ) );
}
...
TEST_F( _TEST_TITLE_, AssignmentOperatorWorks ) {
    ...
    EXPECT_TRUE( test_atmos2.get_property_units( "T" )->equals( *test_atmos2.get_property_units( "T" ) ) );
    EXPECT_TRUE( test_atmos2.get_property_units( "U" )->equals( *test_atmos2.get_property_units( "U" ) ) );
    EXPECT_TRUE( test_atmos2.get_property_units( "V" )->equals( *test_atmos2.get_property_units( "V" ) ) );
    for ( size_t i = 0; i < 5; i++ ) {
...
TEST_F( _TEST_TITLE_, DefaultInterpolatorIsNearestNeighbor ) {
    ...
    for (double z = dz; z < 100.0; z += dz) {
        EXPECT_DOUBLE_EQ( test_atmos.get( "T", z-test_dz ), test_atmos.get( "T", z+test_dz ) );
    }
}
...
TEST_F( _TEST_TITLE_, NewInterpolatorIsUsedCorrectly ) {
    double dz = 0.1;
    double half = dz * 0.5;
    test_atmos.set_interpolator( NCPA::interpolation::interpolator_type_t::LANL_LINEAR );
    for (double z = dz; z < 100.0; z += dz) {
        EXPECT_NEAR( test_atmos.get( "T", z-half ), 0.5*(test_atmos.get( "T", z ) + test_atmos.get("T", z-dz )  ), 0.5 );
    }
}
