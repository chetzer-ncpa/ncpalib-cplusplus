...
namespace NCPA::math {
    ...
    */
    ...
    @param out The shifted array.  Can be the same as either input to perform
    in-place.
    */
    template<typename T>
    ...
    */
    ...
    /**
     * Finds and returns the indices of the array elements before and after the
     * supplied value.
     */
    template<typename T>
    ...
    */
    ...
    std::vector<std::complex<double>>
        double2complex( const std::vector<double> &in ) {
     ...
     */
    std::vector<std::complex<double>>
        double2complex( const std::vector<double> &real,
     ...
     * complex<double>s where one array represents the real parts and the other
     * the imaginary parts.
     * @brief Converts a double array to a complex<double> array
     * @param n The size of the arrays
    ...
    */
    ...
    std::vector<T> random_numbers( size_t N, T minrange, T maxrange ) {
        ...
        std::uniform_int_distribution<T> distribution( minrange, maxrange );
        for ( size_t i = 0; i < N; i++ ) {
    ...
    std::vector<T> random_numbers( size_t N, T minrange, T maxrange ) {
        ...
        std::random_device rd;
        std::mt19937 generator( rd() );
        std::uniform_real_distribution<T> distribution( minrange, maxrange );
        for ( size_t i = 0; i < N; i++ ) {
            randn.push_back( distribution( generator ) );
        }
        return randn;
    ...
    */
    ...
    std::complex<T> random_number( T minreal, T maxreal, T minimag,
                                   T maximag ) {
        return random_numbers<T>( 1, minreal, maxreal, minimag, maximag )[ 0 ];
    }

    /**
    @brief Evaluate a polynomial from its coefficients.
    @param coeffs The coefficients in increasing order starting with power 0
    (i.e. the constant).
    ...
    */
    ...
    T add_vectors( T &v1, T &v2 ) {
        ...
        std::transform( v1.cbegin(), v1.cbegin() + N, v2.cbegin(), v3.begin(), std::plus<>{});
        return v3;
    }
    ...
    */
    template<typename T>
    void add_vectors( size_t N, T *v1, T *v2, T *&v12 ) {
        T *tempvec = NCPA::math::zeros<T>( N );
        for ( size_t i = 0; i < N; i++ ) {
            tempvec[ i ] = v1[ i ] + v2[ i ];
        }
    ...
    /**
    Performs element-wise array division.  If arrays are different lengths, all
    values beyond the length of the shorter vector will be zero.
    ...
    */
    ...
    T divide_vectors( T &v1, T &v2 ) {
        ...
        std::transform( v1.cbegin(),
                        v1.cbegin() + std::min<size_t>( v1.size(), v2.size() ),
                        v2.cbegin(), v3.begin(), std::divides<> {} );
        return v3;
    }
    ...
    @param v1 The array to divide.
    @param v2 The array to divide by.
    @param v12 The array to hold the quotient.  Can be the same as either input
    array, in which case the values are replaced.
    */
    template<typename T>
    void divide_vectors( size_t N, T *v1, T *v2, T *&v12 ) {
        T *tempvec = NCPA::math::zeros<T>( N );
        for ( size_t i = 0; i < N; i++ ) {
            tempvec[ i ] = v1[ i ] / v2[ i ];
        }
    ...
    /**
    Performs element-wise array multiplication.  If arrays are different lengths, all values
    beyond the length of the shorter vector will be zero.
    ...
    */
    ...
    T multiply_vectors( T &v1, T &v2 ) {
        ...
        std::transform( v1.cbegin(),
                        v1.cbegin() + std::min<size_t>( v1.size(), v2.size() ),
                        v2.cbegin(), v3.begin(), std::multiplies<> {} );
        return v3;
    }
    ...
    @param v1 The first array to multiply.
    @param v2 The second array to multiply.
    @param v12 The array to hold the product.  Can be the same as either input
    array, in which case the values are replaced.
    */
    template<typename T>
    void multiply_vectors( size_t N, T *v1, T *v2, T *&v12 ) {
        T *tempvec = NCPA::math::zeros<T>( N );
        for ( size_t i = 0; i < N; i++ ) {
            tempvec[ i ] = v1[ i ] * v2[ i ];
        }
    ...
    */
    template<typename T, typename U>
    void scale_vector( size_t N, U *in, T factor, U *&out ) {
    ...
    */
    template<typename T, typename U>
    void scale_vector( size_t N, U *in, T factor ) {
        for ( size_t i = 0; i < N; i++ ) {
            in[ i ] *= factor;
        }
    }
    ...
    */
    template<typename T>
    void subtract_vectors( size_t N, T *v1, T *v2, T *&v12 ) {
        T *tempvec = NCPA::math::zeros<T>( N );
        for ( size_t i = 0; i < N; i++ ) {
            tempvec[ i ] = v1[ i ] - v2[ i ];
        }
