...
#define EXPECT_ARRAY_EQ( N, AR1, AR2 ) { \
    for (auto i = 0; i < N; i++) { \
        EXPECT_EQ( AR1[i], AR2[i] ) << "Arrays differ at index " << i; \
    } \
}

#define EXPECT_ARRAY_DOUBLE_EQ( N, AR1, AR2 ) { \
    for (auto i = 0; i < N; i++) { \
        EXPECT_DOUBLE_EQ( AR1[i], AR2[i] ) << "Arrays differ at index " << i; \
    } \
}
...
#define EXPECT_ARRAY2D_EQ( M, N, AR1, AR2 ) { \
    for (auto i = 0; i < M; i++) { \
        for (auto j = 0; j < N; j++) { \
            EXPECT_EQ( AR1[i][j], AR2[i][j] ) << "Arrays differ at index " << i << "," << j; \
        } \
    } \
}
...
#define EXPECT_ARRAY2D_DOUBLE_EQ( M, N, AR1, AR2 ) { \
    for (auto i = 0; i < M; i++) { \
        for (auto j = 0; j < N; j++) { \
            EXPECT_DOUBLE_EQ( AR1[i][j], AR2[i][j] ) << "Arrays differ at index " << i << "," << j; \
        } \
    } \
}
...
#define EXPECT_ARRAY3D_EQ( M, N, P, AR1, AR2 ) { \
    for (auto i = 0; i < M; i++) { \
        for (auto j = 0; j < N; j++) { \
            for (auto k = 0; k < P; k++) { \
                EXPECT_EQ( AR1[i][j][k], AR2[i][j][k] ) << "Arrays differ at index " << i << "," << j << "," << k; \
            } \
        } \
    } \
}
...
#define EXPECT_ARRAY3D_DOUBLE_EQ( M, N, P, AR1, AR2 ) { \
    for (auto i = 0; i < M; i++) { \
        for (auto j = 0; j < N; j++) { \
            for (auto k = 0; k < P; k++) { \
                EXPECT_DOUBLE_EQ( AR1[i][j][k], AR2[i][j][k] ) << "Arrays differ at index " << i << "," << j << "," << k; \
            } \
        } \
    } \
}
