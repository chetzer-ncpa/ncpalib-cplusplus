...
namespace NCPA {
    namespace linear {
        ...
        NCPA_LINEARALGEBRA_DECLARE_SPECIALIZED_TEMPLATE  //
            ...
            public:
                using band_diagonal_matrix<ELEMENTTYPE>::get;
                ...
                using band_diagonal_matrix<ELEMENTTYPE>::set_safe;
                ...
                symmetric_matrix( size_t nrows, size_t ncols,
                                  size_t n_offdiags,
                                  bool has_ones_on_last_diagonals ) {
                    if (nrows != ncols) {
                ...
                symmetric_matrix( size_t nrows, size_t ncols,
                    ...
                    symmetric_matrix<ELEMENTTYPE>(
                        nrows, ncols, 0, has_ones_on_last_diagonals ) {}
                ...
                virtual symmetric_matrix<ELEMENTTYPE>& add(
                    ...
                    if (!is_this_subclass( babs )) {
                        return dynamic_cast<symmetric_matrix<ELEMENTTYPE>&>(
                    ...
                    for (size_t ind1 = 0; ind1 < both; ind1++) {
                        this->internal( ind1 + 1 ) = NCPA::arrays::add_vectors(
                            this->internal( ind1 + 1 ),
                            NCPA::arrays::scale_vector(
                                b->internal( ind1 + 1 ), modifier ) );
                    }
                    ...
                    for (size_t ind1 = _n_offdiag - add_on;
                        ...
                        this->internal( ind1 ) = NCPA::arrays::scale_vector(
                            this->internal( ind1 ), modifier );
                    }
                    this->internal( 0 ) = NCPA::arrays::add_vectors(
                        this->internal( 0 ),
                        NCPA::arrays::scale_vector( b->internal( 0 ),
                                                    modifier ) );
                    return *this;
                }
                ...
                virtual std::unique_ptr<abstract_vector<ELEMENTTYPE>> get_row(
                    ...
                    int ind1, ind2;
                    v->set( row, this->internal( 0, row ) );
                    for (int offd = 1; offd <= _n_offdiag; offd++) {
                        if (rowcol2internal( row, row + offd, ind1, ind2 )
                            == diagonal_index_status_t::VALID) {
                            v->set( row + offd, this->internal( ind1, ind2 ) );
                        }
                        if (rowcol2internal( row, row - offd, ind1, ind2 )
                            == diagonal_index_status_t::VALID) {
                            v->set( row - offd, this->internal( ind1, ind2 ) );
                ...
                virtual diagonal_index_status_t internal2rowcol(
                    ...
                    row = (int)ind2;
                    if (ind1 >= this->contents().size() || col < 0
                        || col >= this->columns() || row >= this->rows()) {
                ...
                virtual bool is_identity() const override {
                    ...
                    for (size_t i = 0; i < this->internal( 0 ).size(); i++) {
                        if (this->internal( 0, i )
                            != NCPA::math::one<ELEMENTTYPE>()) {
                            return false;
                        }
                    }
                ...
                virtual diagonal_index_status_t rowcol2internal(
                    ...
                    if (col < row) {
                        return this->rowcol2internal( col, row, ind1, ind2 );
                    }
                ...
                virtual symmetric_matrix<ELEMENTTYPE>& set(
                    ...
                    while (rowcol2internal( row, col, ind1, ind2 )
                        ...
                        this->_add_offdiagonal(
                            (size_t)( ind1 - this->contents().size() + 1 ) );
                    }
                    this->set_internal( ind1, ind2, val );
                ...
                virtual symmetric_matrix<ELEMENTTYPE>& set(
                    ...
                    for (auto i = 0; i <= _n_offdiag; i++) {
                        this->internal( i ).assign( diag.begin(), diag.end() );
                ...
                virtual symmetric_matrix<ELEMENTTYPE>& set_column(
                    ...
                    int icol = (int)col;
                    for (int row = icol;
                         row >= std::max( icol - (int)_n_offdiag, 0 ); row--) {
                ...
                virtual symmetric_matrix<ELEMENTTYPE>& set_diagonal(
                    ...
                    for (size_t i = 0; i < nvals; i++) {
                        this->set_internal( offset, i, vals[ i ] );
                    }
            ...
            protected:
                ...
                void _add_offdiagonal( size_t n = 1 ) {
                    if (n == 0) return;
                ...
                virtual symmetric_matrix<ELEMENTTYPE>& _add(
                    ...
                    newcontents[ 0 ] = NCPA::arrays::add_vectors(
                        this->internal( 0 ), NCPA::arrays::scale_vector(
                                                 b.internal( 0 ), modifier ) );
                    ...
                    for (size_t n = 1; n < new_n_offdiag; n++) {
                        if (n <= _n_offdiag && n <= b._n_offdiag) {
                            newcontents[ n ] = NCPA::arrays::add_vectors(
                                this->internal( n ),
                                NCPA::arrays::scale_vector( b.internal( n ),
                                                            modifier ) );
                        } else if (n <= _n_offdiag) {
                            newcontents[ n ] = this->internal( n );
                        } else if (n <= b._n_offdiag) {
                            newcontents[ n ] = NCPA::arrays::scale_vector(
                                b.internal( n ), modifier );
                        }
                    }
