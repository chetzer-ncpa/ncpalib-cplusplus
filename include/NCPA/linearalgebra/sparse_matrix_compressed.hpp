...
NCPA_LINEARALGEBRA_DECLARE_FRIEND_FUNCTIONS(
    NCPA::linear::sparse_matrix_compressed, ELEMENTTYPE );
...
namespace NCPA {
    namespace linear {
        ...
        NCPA_LINEARALGEBRA_DECLARE_SPECIALIZED_TEMPLATE  //
            class sparse_matrix_compressed<ELEMENTTYPE,
            ...
            public:
                ...
                sparse_matrix_compressed(
                    sparse_matrix_compressed<ELEMENTTYPE>&& source ) noexcept :
                    sparse_matrix_compressed<ELEMENTTYPE>() {
                    ::swap( *this, source );
                }
                ...
                virtual abstract_matrix<ELEMENTTYPE>& as_base_class()
                    override {
                ...
                virtual abstract_matrix<ELEMENTTYPE>& _set_indexed_value(
                    ...
                    size_t i_end   = _pointer_array[ pointer_axis_index + 1 ];
                    ...
                    return _insert_indexed_value( i_end, indexed_axis_index,
                                                  val );
                }
                ...
                virtual abstract_matrix<ELEMENTTYPE>& _insert_indexed_value(
                    ...
                    _indexed_array.insert( _indexed_array.begin() + offset,
                                           new_index );
                    _value_array.insert( _value_array.begin() + offset, val );
                ...
                virtual std::unique_ptr<abstract_vector<ELEMENTTYPE>>
                    ...
                    std::unique_ptr<abstract_vector<ELEMENTTYPE>> v(
                        new sparse_vector<ELEMENTTYPE>( _n_indexed_axis ) );
                ...
                virtual std::unique_ptr<abstract_vector<ELEMENTTYPE>>
                    ...
                    std::unique_ptr<abstract_vector<ELEMENTTYPE>> v(
                        new sparse_vector<ELEMENTTYPE>( _n_pointer_axis ) );
                ...
                virtual abstract_matrix<ELEMENTTYPE>& resize(
                    ...
                    sparse_matrix_compressed<ELEMENTTYPE> newmat(
                        rows, cols, this->axis() );
                    size_t counter = 0;
                    for ( size_t i = 0; i < std::min( rows, _n_pointer_axis );
                          i++ ) {
                        for ( size_t j = _pointer_array[ i ];
                              j < std::min( cols, _pointer_array[ i + 1 ] );
                              j++ ) {
                ...
                abstract_matrix<ELEMENTTYPE>& _swap_pointer_indices(
                    ...
                    size_t j;
                    std::vector<ELEMENTTYPE> valbuffer_lower, valbuffer_upper;
                    std::vector<size_t> indbuffer_lower, indbuffer_upper;
                    ...
                    _indexed_array.erase(
                        ...
                        _indexed_array.begin() + _pointer_array[ upper + 1 ] );
                    ...
                    _value_array.erase(
                        ...
                        _value_array.begin() + _pointer_array[ upper + 1 ] );
                    _value_array.insert(
                    ...
                    _indexed_array.erase(
                        ...
                        _indexed_array.begin() + _pointer_array[ lower + 1 ] );
                    _indexed_array.insert(
                    ...
                    _value_array.erase(
                        ...
                        _value_array.begin() + _pointer_array[ lower + 1 ] );
                    _value_array.insert(
                    ...
                    for ( j = lower + 1; j <= upper; j++ ) {
                        _pointer_array[ j ]
                            = (size_t)( (int)_pointer_array[ j ] + size_diff );
                    }
                    return this->as_base_class();
                }
                    ...
                    for ( size_t i = 0; i < _n_pointer_axis; i++ ) {
                        ...
                        if ( havelower && haveupper ) {
                            ...
                            _value_array.erase( _value_array.begin()
                                                + upper_ind );
                            _value_array.insert(
                                _value_array.begin() + lower_ind + 1, lower );
                        }
                    }
                    return this->as_base_class();
                }
                ...
                virtual abstract_matrix<ELEMENTTYPE>& change_axis(
                    ...
                    for ( size_t i = 0; i < _pointer_array.size() - 1; i++ ) {
                        for ( size_t j = _pointer_array[ i ];
                              j < _pointer_array[ i + 1 ]; j++ ) {
                            if ( this->axis() == compression_axis_t::ROWS ) {
                ...
                abstract_matrix<ELEMENTTYPE>& _remove_element(
                    ...
                    for ( size_t j = _pointer_array[ pointer_index ];
                          j < _pointer_array[ pointer_index + 1 ]; j++ ) {
                        if ( _indexed_array[ j ] == indexed_index ) {
                            _indexed_array.erase( _indexed_array.begin() + j );
                ...
                virtual abstract_matrix<ELEMENTTYPE>& zero() override {
                    if ( this->axis() == compression_axis_t::ROWS ) {
                        this->_set_dimensions( this->rows(), this->columns() );
                    } else {
                        this->_set_dimensions( this->columns(), this->rows() );
                    }
                    return this->as_base_class();
                }
...
template<typename T>
static void swap( NCPA::linear::sparse_matrix_compressed<T>& a,
                  NCPA::linear::sparse_matrix_compressed<T>& b ) noexcept {
    using std::swap;
